...
struct node1                            //дл€ проверки корректности скобок
{
    char data;
    node1 * next;
};
...
double RectangleMethod(double a, double b, int n)                //метод пр€моугольников
{
...
double GaussMethod(double a, double b, int n)                    //метод √аусса
{
	double s = 0, h = (b-a) / (double)n ;
	for (int i = 0 ; i < n-1 ; i++)
...
double MonteKarloMethod(double a, double b, int n)                    //метод ћонте- арло
{
	double min, max, f, h;
